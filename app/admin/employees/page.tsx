...
export default function EmployeesList() {
  ...
  const [formData, setFormData] = useState({
    ...
    role: "Employee",
    status: "Active",
    password: "", // Optional password reset field
  });

  // Consolidated Single Clean Fetch Engine
  ...
  // Filter Engine (Completely removed Department parameters)
  ...
  // Handler to open and fill Modal input values
  const handleEditClick = (user: any) => {
    ...
    setFormData({
      ...
      password: "", // Keep empty initially for safety
    });
  ...
  const handleUpdateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // If password field is left empty, don't include it in payload so it doesn't modify the existing one
      ...
      fetchUsers(); // Refresh background UI metrics instantly
    } catch (err: any) {
      console.error("Update request failed:", err);
      alert(
        "Failed to update data: " +
          (err.response?.data?.message || "Server configuration error"),
      );
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-[#F8FAFC] font-sans text-slate-800 relative">
                  ...
                  {loading ? (
                    ...
                    <tr>
                      <td
                        ...
                        className="px-6 py-8 text-center text-slate-500 font-medium"
                      >
                        No system records match search query.
                      </td>
                    </tr>
                  ) : (
      ...
      {/* --- REUSABLE EDIT MANAGEMENT POP-UP MODAL --- */}
      {isEditModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all animate-fade-in">
          <div className="bg-white rounded-2xl border border-slate-200 w-full max-w-lg shadow-2xl overflow-hidden flex flex-col transform transition-transform scale-100">
            {/* Header Frame */}
            <div className="p-6 border-b border-slate-100 bg-slate-50/60 flex justify-between items-center">
              ...
              <button
                ...
                className="text-slate-400 hover:text-slate-600 p-1 bg-white border border-slate-200 rounded-full shadow-sm hover:shadow transition-all"
              >
                <svg
                  className="w-5 h-5"
                  fill="none"
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 outline-none"
                  >
              ...
              {/* SECURITY PASSWORD OVERWRITE OPTION */}
