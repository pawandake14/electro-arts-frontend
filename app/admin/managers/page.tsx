...
import Sidebar from "@/components/Sidebar"; // Adjust if you use AdminSidebar
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await axios.get(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/users?role=Manager`,
      ...
      <main className="flex-1 md:ml-64 flex flex-col h-screen overflow-hidden w-full">
        <header className="bg-white border-b border-slate-200 h-16 flex items-center justify-between px-4 md:px-8 flex-shrink-0 z-10">
          <div className="flex items-center flex-1">
            ...
            <h1 className="text-xl font-bold text-slate-800">All Managers</h1>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 custom-scrollbar">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
            ...
            <div className="text-sm text-slate-500 font-medium">
              Showing {filteredUsers.length} managers
            </div>
          </div>
